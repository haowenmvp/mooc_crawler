...
class PlatformProcessor:
    def __init__(self, processor: Processor):
        self.platform = "优学院（人民网公开课）"
        self.processor = processor

    def process_table_old_course(self):
        no_group_id_sql = "select * from course_list_info where platform = '优学院（人民网公开课）' and course_group_id = ''"
        ...
        for no_group_id_course in no_group_id_res:
            ...
            cid = no_group_id_course[1].split('?courseID=', 1)[1]  # no_group_id_course[1]为url
    ...
    inner join (select course_group_id,max(term_id) as term_count from course_list_info where platform=\"优学院（人民网公开课）\"  and course_group_id<>'' group by course_group_id)b \
