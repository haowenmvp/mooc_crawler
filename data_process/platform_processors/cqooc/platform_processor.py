...
from crawler.fetcher.cqooc.list_fetcher import ListFetcher
from persistence.model.basic_info import CourseListInfo
...
class ProcessorPlatformCourse(BaseProcessorPlatformCourse):
    def __init__(self):
        super().__init__()
        # 平台名不输入， 自己填写
        self.platform = '重庆高校在线开放课程平台（重庆市教育委员会）'
...
class PlatformProcessor:
    ...
    def process_table_old_course(self):
        no_group_id_sql = "select * from course_list_info where platform = '重庆高校在线开放课程平台（重庆市教育委员会）' and course_group_id = ''"
    ...
    def init_table(self):
        sql = "select a.course_group_id,a.course_name,a.school,a.teacher,a.platform,a.status from course_list_info a \
            inner join (select course_group_id from course_list_info where platform=\"重庆高校在线开放课程平台（重庆市教育委员会）\"  and course_group_id<>'')b \
