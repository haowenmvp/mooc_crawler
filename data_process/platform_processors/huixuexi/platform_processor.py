...
class ProcessorPlatformCourse(BaseProcessorPlatformCourse):
    def __init__(self):
        super().__init__()
        # 平台名不输入， 自己填写
        self.platform = '国家开放大学出版社有限公司（荟学习网）'
    ...
    def process_platform(self, course_list_info=None):
        ...
        platform_info.platform = '国家开放大学出版社有限公司（荟学习网）'
...
if __name__ == '__main__':
    ...
    for item in course_list_info:
        if item.platform == "国家开放大学出版社有限公司（荟学习网）":
            course_list_info2.append(item)
    test = BaseProcessorPlatformCourse()
...
class PlatformProcessor(BaseProcessor):
    ...
    inner join (select course_group_id,max(term_id) as term_count from course_list_info where platform=\"国家开放大学出版社有限公司（荟学习网）\" and course_group_id != '' group by course_group_id)b \
    ...
    def process_table_old_course(self):
        sql = "update course_list_info inner join " \
              "(select course_id, course_name, school, teacher, platform from course_list_info " \
              "where (platform = '国家开放大学出版社有限公司（荟学习网）') AND (course_group_id = '')) a " \
              "on course_list_info.course_id = a.course_id " \
              "set course_group_id = MD5(CONCAT(a.course_name,a.teacher,a.school,a.platform));"
