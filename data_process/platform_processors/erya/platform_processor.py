...
class PlatformProcessor:
    ...
    def process_table_old_course(self):
        no_group_id_sql = "select * from course_list_info where platform = '北京超星尔雅教育科技有限公司' and course_group_id = ''"
        ...
        for no_group_id_course in no_group_id_res:
            ...
            cid_back = no_group_id_course[1].split('course/', 1)[1]  # no_group_id_course[1]为url
            cid = cid_back.split('.html', 1)[0]
    ...
    inner join (select course_group_id,max(term_id) as term_count from course_list_info where platform=\"北京超星尔雅教育科技有限公司\"  and course_group_id<>'' group by course_group_id)b \
