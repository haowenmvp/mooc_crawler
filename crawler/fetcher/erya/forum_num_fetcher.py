...
class ForumNumFetcher(BaseFetcher):
    ...
    def crawl(self, course_url):
        course_id = course_url.split("/")[-1].split(".")[0]
        url = "https://mooc1.chaoxing.com/moocAnalysis/courseStatistic?courseId=" + course_id + "&clazzIds=0&batchNum=1&disableCache=false"
        headers = {
    ...
    def run(self, data=None, login_info=None):
        ...
        for one_course in data:
            ...
            term = one_course[2]
            try:
                num = self.crawl(course_url)
                # print(course_url, num)
            except:
                print(course_url, "crawl forum_num error")
                continue
            forum_num_info = ForumNumInfo()
            forum_num_info.platform = '北京超星尔雅教育科技有限公司'
            forum_num_info.course_id = course_id
            forum_num_info.accumulate_forum_num = num
            forum_num_info.forum_num = num
...
if __name__ == '__main__':
    ...
    forum_num_fetcher.crawl("https://mooc1.chaoxing.com/course/200837021.html")
